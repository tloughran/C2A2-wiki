...
24. **Does a stored morphogenetic setpoint model a normative end, or only a habit?** The recasting of Aristotelian virtue as response to an externally-stored setpoint (PRS-116) borrows the planarian two-headed line as its manipulable substrate, and the borrowing works descriptively: the target state is held outside the code and is writable from outside. But Levin's setpoints are **descriptive throughout** — an induced two-headed target is a stored goal, and cancer is reversion to a smaller light cone — so the model describes vice exactly as well as virtue. The question is whether anything on Levin's side can supply the normative direction, or whether it must always be imported (from *ergon* and eudaimonia, or from an ultimate end). *(added 2026-09-09; PRS-116; CROSS-131)*
25. **Is the awareness-indexed reading of "cognitive light cone" a legitimate extension or an equivocation?** PRS-116 stipulates a stricter reading under which a goal's source counts as *external* when it is not cognitively available to the agent. On Levin's own usage a bioelectrically stored target is **inside** the collective's light cone. The stipulation is declared rather than smuggled, which is the system working — but it makes the term equivocal across the traditions CROSS-131 joins, and it is now the fourth such term in this network. *(added 2026-09-09; PRS-116; FINDING-083)*
