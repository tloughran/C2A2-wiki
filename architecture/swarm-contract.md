...
Standing guidance every agent in the C2A2 swarm inherits. Ground-truth copy lives here; `wiki/architecture/swarm-contract.md` is the Obsidian-visible mirror.
...
Every new agent's SKILL.md prompt should be written so that an outside reviewer could trace each of these six properties to a concrete behavior in the prompt. Where an agent's task forces an override (e.g., a one-shot migration agent that genuinely cannot run dry first), the override is named in the SKILL.md under a `## Contract overrides` section, with a one-line rationale.
...
- [[../CLAUDE.md]] — project standing instructions (Constitutional rules)
- Janitor: `scripts/janitor.py`
- Memory index: `~/Library/Application Support/Claude/.../memory/MEMORY.md`
