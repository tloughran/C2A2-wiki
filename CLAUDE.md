...
- **Deterministic** (`scripts/voice_faq.py`): `scan` reads `wiki/voice_guide/knowledge/*.md` — the canonical source since `bd33ef7` (2026-07-22); it no longer parses `explorer.html` — keys each file by its `state_key` frontmatter, hashes it, diffs against `voice_faq/state.json` → new/changed/unchanged/removed. `status` prints total/target/deficit/phase + per-feature counts (thinnest first). `merge <qa.json>` is **ADDITIVE** — appends authored Q&A per feature, deduped by normalized question (never overwrites); validates (known keys, non-empty q/a, new features need ≥3 seed pairs); writes `wiki/voice_guide_faq.json` + `voice_faq/report.md` + updates state. Phase = `ramp` while total < 100, else `steady`.
...
- ⚠️ **The key migration is unfinished, and `status` reads 0 because of it.** `bd33ef7` re-keyed features from per-tab filenames (`prs_3d.html`) to per-affordance-state `state_key`s (`sociogram.graph.node_selected`) and ported **only the Sociogram slice** — `knowledge/` holds 4 files. The published `wiki/voice_guide_faq.json` still carries the **102 pairs across 14 old keys** seeded 2026-07-18, so the guide still answers; but `scan` reports 4 new / 14 removed / 0 unchanged, `status` reports total 0 / phase `ramp`, and `c2a2-voice-faq-weekly` has produced nothing since (it ran 2026-08-02 and wrote no state). Finishing it means authoring the remaining 13 tabs' knowledge files and re-homing the 102 pairs by hand — one old bucket splits across several states, so it is not a rename table.
- **`merge` refuses to drop Q&A** (added 2026-08-05, `scripts/test_voice_faq.sh`, 19 assertions). The merge is additive *within* a key, but the output is rebuilt from the inventory, so any key holding pairs that the inventory no longer names would silently vanish. It now exits 1 and lists each key with its pair count. `--allow-drop` is the migration's way to say "already re-homed"; a permitted drop is still named in `voice_faq/report.md`. Only the pre-existing "new feature needs ≥3 seed pairs" gate fires first — clear that and this one is what stands between the 102 pairs and an overwrite.
...
bash scripts/test_voice_faq.sh                           # 19 assertions on the drop guard
```
