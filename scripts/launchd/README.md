...
| `com.c2a2.voice-shell-check` | `check_voice_shell.sh` — the CCL shell suite (`test_voice_shell.cjs`, real Chrome over CDP) gated on a tested surface being newer than the last verdict; writes `scheduler/voice_shell.json` + `voice_shell.FAILED`, exit 3 = RED | daily 05:20 |
